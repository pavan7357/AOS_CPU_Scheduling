def srtf(process_list):
    ...
    while process_list:  
        ...
        if not available:
            ...
            min_burst = min([p[1] for p in available])
            next_event = min([p for p in available if p[1] == min_burst], key=lambda x: x[0] + x[1])
            process = next_event
            gantt.append((process[2], t, t + 1))
            rem_burst = process[1] - 1
            process[1] = rem_burst
            t += 1
            if rem_burst == 0:
